...
// URLInfo - структура для хранения информации об URL из БД
type URLInfo struct {
	OriginalURL string
	IsDeleted   bool
}

// UserURL - структура для ответа хендлера GetUserURLs
type UserURL struct {
	ShortURL    string `json:"short_url"`
	OriginalURL string `json:"original_url"`
}

// DeleteFlaggedURLs физически удаляет URL, помеченные флагом del_flag
...
// GetURLInfo получает OriginalURL и IsDeleted по shortID
...
// BatchDelete помечает слайс shortID флагом del_flag для конкретного userID
...
// AddOne добавляет один URL в БД, проверяя на конфликт
...
// AddOneTransaction добавляет один URL в БД в рамках существующей транзакции
...
// GetUserURLs возвращает все URL (не помеченные удаленными) для конкретного
// userID
